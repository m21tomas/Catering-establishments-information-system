    ...
    const handleSubmit = (event) => {
        ...
        if (duomenys.file !== '' || duomenys.image !== '') {
            ...
            if (image.disk) {
                ...
                let binaryFileType = blobPromise.type.split('/').pop();
                const fileName = `Internet-Image-${dateTime}.${binaryFileType}`;
                console.log("fileName: ", fileName, " blob type: ", blobPromise.type, " binaryFileType: ", binaryFileType)
                console.log("Blob size: ", blobPromise.size);
                // Use the original blobPromise directly
                const file = new File([blobPromise], fileName, {
                    type: blobPromise.type
                });
